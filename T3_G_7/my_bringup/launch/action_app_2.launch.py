...
def generate_launch_description():
    ...
    # Movimento finale del braccio fino a [0.07, 0.1, -3.1, 1.36, 2.05, -0.52, 1.14]
        ...
        actions=[
            ExecuteProcess(
                cmd=[
                    'timeout','12','ros2', 'topic', 'pub', '/arm_controller/joint_trajectory',
                    'trajectory_msgs/msg/JointTrajectory', 
                    '{"joint_names": ["arm_1_joint", "arm_2_joint", "arm_3_joint", "arm_4_joint", "arm_5_joint", "arm_6_joint", "arm_7_joint"], "points": [{"positions": [0.07, 0.1, -3.1, 1.36, 2.05, -0.52, 1.14],"time_from_start": {"sec": 2, "nanosec": 0}}]}'
                ]
            )
        ]
    )

    # Nodo 4: (Motion Planner per l'inversione cinematica con LbD) dopo 83 secondi dall'inizio (subito dopo il movimento del braccio)
    motion_planner = TimerAction(
        period=83.0,
        actions=[
            Node(
                package="head_scan",
                executable="lbd_motion_planner"
            )
        ]
    )

    # Nodo 5: (State Machine con LbD) dopo 90 secondi dall'inizio (7 secondi dal precedente)
    state_machine = TimerAction(
        period=90.0,
        actions=[
            Node(
                package="head_scan",
                executable="lbd_state_machine"
            )
        ]
    )
